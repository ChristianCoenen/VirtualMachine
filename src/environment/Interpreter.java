...
/**
 * A interpreter class, which inteprets machinecode in the memory addresses of a
 * virtual machine.
 *
...
public class Interpreter implements Globals {
	...
	private static void interpretLine(short opCode, VM v) {
		...
		case MOV: {
			...
			// writes the value of the register ry into the memory with address
			// given by the value in register rx
			if (toMem && !fromMem) {
				v.memoryAddress[v.register[rx]] = v.register[ry];
			}
			// writes the value of the memory with address given by the value in
			// register ry in the register rx
			else if (!toMem && fromMem) {
				v.register[rx] = v.memoryAddress[v.register[ry]];
			}
			// writes the value of the memory with address given by the value in
			// register ry in the memory with address given by the value in
			// register rx
			else if (toMem && fromMem) {
		...
		// checks for divide-by-zero and writes the solution into the register
		// rx
		case DIV: {
			if (v.register[ry] != 0) {
				v.register[rx] /= v.register[ry];
		...
		case JIZ: {
			if (v.register[0] == 0) {
				v.pC = data;
				// special-case decrease the counter because normally we would
				// do nothing with pC here but we are increasing it in the end for all commands
				v.pC--;
			}
			break;
		}
